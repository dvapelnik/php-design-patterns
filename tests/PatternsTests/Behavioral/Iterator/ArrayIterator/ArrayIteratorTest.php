...
class ArrayIteratorTest extends PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        $this->_reflectedArrayProp = new ReflectionProperty(
            '\Patterns\Behavioral\Iterator\ArrayIterator\ArrayIterator',
            '_array');
    ...
    {
        $iterator = new ArrayIterator($this->_array);
    ...
    public function testFirst(IteratorInterface $iterator)
    {
        ...
        $this->assertEquals(0, $this->_reflectedCurrentIndexProp->getValue($iterator));
    ...
    {
        for ($i = 0, $this->_iterator->first();
             $i < count($this->_array) && !$this->_iterator->isDone();
